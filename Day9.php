...
function addNumberReverse($array)
{
    ...
    for ($i = 0; $i < count($array); $i++) {
        ...
        if ($i != 0) {
            $val_to_add = ($array[$i][0] - $array[$i - 1][0]);
            array_unshift($array[$i], $val_to_add); // A AJOUTER LA SOUSTRACTION
        }
        $array[0][] = 0;
        //echo "String $i: " . implode(" ", $array[$i]) . "<br />";
    }
    $answer2 += $array[$last_key][0];
}


echo '<pre> <strong>Partie 1</strong> : ' . $answer1 . '</pre> ';
echo '<pre> <strong>Partie 2</strong> : ' . $answer2 . '</pre> ';
